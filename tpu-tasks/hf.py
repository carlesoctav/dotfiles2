...
def stream_one(
    ...
    url = hf_hub_url(repo, path, subfolder = subfolder, repo_type=repo_type)
...
def main():
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print(f"Usage: {sys.argv[0]} [repo] [gcs_path] [repo_type] [subfolder (optional)]")
        sys.exit(1)
    ...
    with tqdm(total=len(files), desc="Total files") as total_bar:
        with ThreadPoolExecutor(max_workers=8) as ex:
            futs = [ex.submit(stream_one, path, subfolder, repo, repo_type, bucket, prefix, hf_token) for path in files]
