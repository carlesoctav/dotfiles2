...
# Description:
# gcs_checkpoint.sh [gcs_bucket] [mount_path] [file_path]
# Mounts GCS bucket with aiml-checkpointing profile optimized for checkpoint saves
...
# Check arguments
if [[ $# -lt 2 ]]; then
  echo "Usage: gcs_checkpoint.sh [gcs_bucket] [mount_path] [file_path]"
  ...
  echo ""
  echo "Example: gcs_checkpoint.sh gs://my-checkpoints ~/checkpoints"
  exit 1
fi
...
LOG_FILE="$HOME/gcsfuse_checkpoint_$TIMESTAMP.json"

echo "Mounting $DATASET_GCS_BUCKET to $MOUNT_PATH with aiml-checkpointing profile..."
echo "Log file: $LOG_FILE"

# Mount with aiml-checkpointing profile
# This profile is optimized for checkpoint write operations
gcsfuse --profile=aiml-checkpointing --implicit-dirs --log-severity=debug \
